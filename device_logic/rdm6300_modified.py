    ...
    def _read(self):
        ...
        while not self.quit_reader:
            ...
            if received_bytes and len(received_bytes) > 0:
                #print(int(received_bytes.hex(),16), received_bytes)
                ...
                if recieved_byte == BaseReader._RFID_STARTCODE:
                    if len(self.current_fragment) > 0:
                        ...
                        self.current_fragment = []
                elif recieved_byte == 0xe0 or recieved_byte == 0xf0:
                    if len(self.current_fragment) > 0:
                        #self._process_fragment(self.current_fragment)
                        fragment = int(received_bytes.hex(),16)
                        self.current_fragment.append(fragment)
                        self.card = CardData(
                            value=BaseReader._fragment_to_int(self.current_fragment),
                            checksum='',
                            is_valid=True,
                            type=None
                        )
